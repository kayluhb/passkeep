  ...
  def clippy(text, id, bgcolor='#F5F5F5')
    ...
    html = <<-EOF
      <span class="clippy">
        ...
        <param name="movie" value="/assets/swfs/clippy.swf" />
        <param name="allowScriptAccess" value="always" />
        <param name="quality" value="high" />
        <param name="scale" value="noscale" />
        <param name="FlashVars" value="text=#{text}" />
        <param name="bgcolor" value="#{bgcolor}" />
        <embed src="/assets/swfs/clippy.swf" width="110" height="14" name="clippy-#{id}" quality="high" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" FlashVars="text=#{text}" bgcolor="#{bgcolor}" />
        </object>
      </span>
    EOF
    html.html_safe
